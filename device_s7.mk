...
ifeq ($(TARGET_PREBUILT_KERNEL),)
        LOCAL_KERNEL := device/huawei/s7/kernel
else
...
# Board-specific init
PRODUCT_COPY_FILES += \
    device/huawei/s7/init.qsd8k_s7.rc:root/init.qsd8k_s7.rc 
...
PRODUCT_COPY_FILES += \
    device/huawei/s7/dhcpcd.conf:system/etc/dhcpcd/dhcpcd.conf \
    device/huawei/s7/vold.fstab:system/etc/vold.fstab \
    device/huawei/s7/wpa_supplicant.conf:system/etc/wifi/wpa_supplicant.conf \
    device/huawei/s7/surf_keypad.kl:system/usr/keylayout/surf_keypad.kl

$(call inherit-product, build/target/product/full.mk)

PRODUCT_BUILD_PROP_OVERRIDES += BUILD_UTC_DATE=0
PRODUCT_NAME := generic_s7
PRODUCT_DEVICE := s7
